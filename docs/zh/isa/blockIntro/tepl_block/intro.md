...
[数据供给] Tile寄存器阵列 → [核心计算] TEPL模版数据块 (硬化计算) → [结果暂存] Tile寄存器阵列
                      ...
                      [控制] 核心调度器 (指令派发)
```
*   **输入与输出：** 其操作数源和结果目标主要是 **Tile 寄存器阵列**。它不直接管理系统内存搬移（这是 TLSU 的职责），而是专注于对驻留 Tile 数据进行计算。
*   **协作关系：** 依赖 TLSU 将数据从内存加载到 Tile 寄存器；计算完成后，结果可由 TLSU 写回或供其他 Tile 操作使用。
...
* **数据移动**：**不直接访问系统内存**。数据输入输出通过 Tile 寄存器阵列和全局寄存器 GGPR 与 TLSU 协同工作。
