...
```asm
TGEMVMX.ACC <LB0:M, LB1:N, LB2:K, DataTypeA, DataTypeB> SrcTile0<.reuse>, SrcTile1<.reuse>, SrcTile2<.reuse>, SrcTile3<.reuse>
```
...
- [BSTART.TGEMVMX.ACC](../../blockIntro/cube_block/header.md) `DataTypeA`
...
- [B.IOT](../../header/B.IOT.md) `SrcTile2<.reuse>, SrcTile3<.reuse>, last`
...
```asm
TGEMVMX.ACC <LB0:1, LB1:32, LB2:64, e3m4, e1m2X2>, T#1.reuse, U#1, M#2, N#2
```
...
1. 本指令读取显式 Local 累加输入 C，并写入显式 Local 目的 Tile D。
2. 本指令是一种模版块，软件只定义块头。
