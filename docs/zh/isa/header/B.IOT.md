...
* **DstTile**: 指定输出 Tile 寄存器的类型：`T`, `U`, `M`, `N`, `ACC``S` 或 `PT`, `PT.FREE`。
...
3. DstTile为 `PT.FREE` 时无Size信息，因此将缺省Size符号。
...
| ------- | ----------| ------- | -----------|
| 0 | 输出到 **T队列** | 4 | 申请 **PT寄存器** |
| 1 | 输出到 **U队列** | 5 | 输出到 **S队列** |
| 2 | 输出到 **M队列** | 6 | 执行 **PT.FREE** 操作 |
| 3 | 输出到 **N队列** | 7 | 本字段无效 |

输出 Tile 寄存器的大小通过立即数 **imm4** 指定，计算公式为：`Size = 16 Byte * (2^imm4)`。imm4 的有效取值范围为 0 到 15，映射关系如下表：
...
```
由于每条B.IOT指令只能编码两输入一输出的Tile 寄存器，因此上述的3个输入Tile 需要编码在两条指令中：
```asm
