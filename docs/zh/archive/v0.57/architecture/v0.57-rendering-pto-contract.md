...
Canonical `v0.57` 渲染 PTO 使用这些载体：

- `BSTART.TEPL` 用于 块头 驱动的模板操作，并分配
  `TileOpcode` 值，
- 用于 tile 移动的规范 `BSTART.TLOAD`、`BSTART.TSTORE` 和 `BSTART.TMOV` 形式
  和瓷砖记忆管道，
...
`v0.57` 当前未为其分配渲染 PTO 选择器目录、
机器可读的语义配置文件，或特定于渲染的描述符契约。

因此：

- 渲染 PTO 不得分配给规范 `v0.57` 中的 `BSTART.FIXP`
  文档、AVS、示例或标准化输出，
...
在规范的 `v0.57` 中，这些名称仅用于规划。它们可以被讨论为
