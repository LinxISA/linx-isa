...
- 在控制流区域周围使用 `BSTART ...` / `BSTOP` 或 `C.BSTART ...` / `C.BSTOP`。
- 区分架构块和编译器基本块。当指令仍属于当前 STD 或 Tile 块时，唯一的顺序
  CFG 边不需要新的 FALL-STD 块头。
- 允许普通标量块体指令跟在 Tile 块头之后。不要在相邻 Tile 块指令之间，或
  Tile 块头与其顺序标量块体之间插入空的 `C.BSTART.STD`。
...
- 在标量控制流目标、非顺序传输、必要的块类型切换、显式提交边界和最终提交
  边界处生成 BSTART。
- Tile 块指令在自身地址提供 BSTART；当该地址是控制流目标时也是如此。
- 不要跨 CFG 汇合、间接边、异常边或块类型切换删除边界。
