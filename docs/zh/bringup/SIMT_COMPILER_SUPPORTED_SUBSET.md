...
选择形式，分组内核可以保留在当前编译器的 `v0.57` 兼容性块体合约内
（`BSTART.MSEQ`、`B.TEXT`、分组 `LB0/LB1`、向量比较和选择），无需
一流的 EXEC 掩码保存/恢复或拆分/重新加入重新收敛。在 consumer 完成
升级并重新验证之前，这些编译器证据不得转移为规范 v0.58 架构证据。
...
`v057_vector` 仍然是面向重放的执行通道的证据，而不是
