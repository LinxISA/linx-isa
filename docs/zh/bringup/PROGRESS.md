...
- `isa/v0.58/linxisa-v0.58.json` 是当前规范 ISA 合约。
- v0.58 切换之前的运行记录仅属于历史兼容性证据，不是 v0.58 发布证明。
- AVS 现在是唯一实时公开的培育合同。
...
|相|状态 |证据|
| --- | --- | --- |
| 1. `v0.57` 兼容性 golden + manual 冻结 | ✅ 历史通过（不得转移到 v0.58） | `python3 tools/isa/build_golden.py --profile v0.57 --check`; `python3 tools/isa/validate_spec.py --profile v0.57` |
...
|门 |状态 |命令 |
| --- | --- | --- |
| v0.57 兼容性 golden/spec 验证 | ✅ 历史通过（不得转移到 v0.58） | `python3 tools/isa/build_golden.py --profile v0.57 --check`; `python3 tools/isa/validate_spec.py --profile v0.57` |
