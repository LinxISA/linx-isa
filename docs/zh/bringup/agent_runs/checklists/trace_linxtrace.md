...
- [ ] ID：TRACE-001 通过 灵犀Trace 合约同步 lint 门。
  命令：`python3 rtl/LinxCore/tools/linxcoresight/lint_trace_contract_sync.py`
...
- [ ] ID：TRACE-002 通过 灵犀Trace 样本棉绒门。
  命令：`bash rtl/LinxCore/tests/test_konata_sanity.sh`
  完成意味着：示例跟踪验证架构/阶段要求。
...
- [ ] ID：TRACE-004 通过 DFX 追踪夜间门。
  命令：`bash rtl/LinxCore/tests/test_konata_dfx_pipeview.sh`
  完成意味着：DFX 跟踪通道验证成功。
  状态：⚪ 未运行 (2026-03-15) - 最新的严格引脚运行使夜间跟踪门禁用 (`RUN_TRACE_NIGHTLY_GATES=0`)。

- [ ] ID：TRACE-005 通过模板追踪夜间门。
  命令：`bash rtl/LinxCore/tests/test_konata_template_pipeview.sh`
  完成意味着：模板跟踪通道验证成功。
  状态：⚪ 未运行 (2026-03-15) - 最新的严格引脚运行使夜间跟踪门禁用 (`RUN_TRACE_NIGHTLY_GATES=0`)。
