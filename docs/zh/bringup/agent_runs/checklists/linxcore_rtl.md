...
- [ ] ID：LC-001 通过阶段/连接棉绒门。
  命令：`bash rtl/LinxCore/tests/test_stage_connectivity.sh`
...
- [x] ID：LC-002 通过操作码奇偶校验门。
  命令：`bash rtl/LinxCore/tests/test_opcode_parity.sh`
  完成意味着：解码/操作码元数据奇偶校验通过。
  状态： ✅ PASS (2026-03-15) - `LinxCore::opcode parity` 在最新的 pin-lane 报告 `2026-03-15-r2-pin` 中为绿色。

- [ ] ID：LC-003 通过跑步者礼宾门。
  命令：`bash rtl/LinxCore/tests/test_runner_protocol.sh`
...
- [ ] ID：LC-004 传递跟踪模式和内存烟门。
  命令：`bash rtl/LinxCore/tests/test_trace_schema_and_mem.sh`
...
- [ ] ID：LC-005 通过 cosim 防烟门。
  命令：`bash rtl/LinxCore/tests/test_cosim_smoke.sh`
  完成意味着：QEMU↔灵犀Core 烟雾 co-sim 通过且快速失败行为完好无损。
  状态：❌失败 (2026-03-15) - 生成的 灵犀Core 更新脚本中止，因为固定树中缺少 `rtl/LinxCore/tools/lib/workspace_paths.sh`（日志：`docs/bringup/gates/logs/2026-03-15-r2-pin/pin/linxcore_cosim_smoke.log`）。- [ ] ID：LC-006 通过 CoreMark 交叉检查夜间门。
  命令：`bash rtl/LinxCore/tests/test_coremark_crosscheck_1000.sh`
...
- [ ] ID：LC-007 通过 CBSTOP 通货膨胀守卫夜间门。
  命令：`bash rtl/LinxCore/tests/test_cbstop_inflation_guard.sh`
