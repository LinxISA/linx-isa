...
当一个编译器基本块仍属于当前架构块时，不需要为它单独生成 BSTART。特别地，
活跃的 Tile 块可以包含普通标量块体指令；第一条成功解码的标量指令进入块体
阶段，但不改变 Tile 块类型。

因此，在唯一的顺序 CFG 边上，如果后续标量指令仍属于当前 STD 或 Tile 块，
编译器可以省略 `BSTART.STD FALL` 或 `C.BSTART.STD FALL`。下一条真正的
BSTART、Tile 块指令或 BSTOP 构成提交边界。

以下情况必须保留显式 BSTART：

- 该地址是标量控制流目标；
- 传输类型为 `COND`、`DIRECT`、`CALL`、`IND`、`ICALL` 或 `RET`；
- 新块需要切换架构块类型；
- 后续指令之前需要显式提交边界；
- 该指令用于结束函数中的最后一个活跃块。

如果控制流目标以 Tile 块指令开始，该 Tile 指令本身就是所需的 BSTART；不要
在它前面插入空的 STD 块。
