...
- B-F4：基于匹配 I-F4 boundary metadata 的 static predictor、long TAGE、
  final IBTB、loop、RAS final check 和统一仲裁；
...
flush 点。B-F4 final record 随每条指令进入 D1；Dispatch 校验 direct/call，
BRU E1 校验 conditional direction 与 indirect/return target，错误统一使用
BRU flush/recover 加 frontend restart。
