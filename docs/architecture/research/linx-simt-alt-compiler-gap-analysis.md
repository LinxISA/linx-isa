...
- the canonical `v0.57` ISA manual
...
- Current canonical design:
  - The `v0.57` manual defines one scalar-uniform control-flow context per group plus EXEC mask `p` (`docs/architecture/isa-manual/src/chapters/03_programming_model.adoc:121-167`).
...
- Current canonical design:
  - `v0.57` explicitly separates block-control predicate state (`BARG.CARG`) from kernel EXEC mask `p` (`docs/architecture/isa-manual/src/chapters/03_programming_model.adoc:109-119`).
...
- Recommended closure direction:
  ...
  - If that is too large for `v0.57`, then codify a strict predication-only subset and ban code shapes like this from canonical lowering.
...
The alternative compiler is not primarily exposing missing single instructions. It is exposing that the current `v0.57` SIMT story is still a partially hardened replay model, while this hash-table kernel wants a more explicit GPU-like execution contract.
