...
* Bare calls use `BSTART.STD CALL, <target>` and preserve `ra`.
* Returning calls materialize the explicit return label in `ra` using an adjacent `SETRET` or `C.SETRET`.
...
before** the call-type block start marker (e.g. `BSTART.STD CALL, <target>` / `C.BSTART CALL`). Tooling and hardware may
treat the `SETRET`+`BSTART` pair as a fused call header; no other instruction may be placed between them.
