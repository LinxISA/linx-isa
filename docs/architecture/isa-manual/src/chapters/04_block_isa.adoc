...
Block types beyond the scalar core use the named TMA and CUBE direct forms plus the raw TEPL Mode/Function form.
Selector space not explicitly assigned by the PTO ISA 0.57.1 catalog is reserved and must not acquire ad hoc semantics.
...
* Typed operation starts are canonical for TMA and CUBE. TEPL uses `BSTART.TEPL Mode, Function, DataType`.
...
v0.57.1 tile header encoding families:
...
* TMA, CUBE, and TEPL name binary encoding families. TMA and CUBE have no generic
...
** `BSTART.TLOAD` -> TMA `Function=0`
** `BSTART.TSTORE` -> TMA `Function=1`
** `BSTART.TMOV` -> TMA `Function=2`
** `BSTART.TPREFETCH` -> TMA `Function=3`
** `BSTART.MGATHER` -> TMA `Function=4`
** `BSTART.MSCATTER` -> TMA `Function=5`
** `BSTART.MGATHER.MASK` -> TMA `Function=6`
** `BSTART.MSCATTER.MASK` -> TMA `Function=7`
** `BSTART.MGATHER.CAS` -> TMA `Function=8`
** `BSTART.TMATMUL` -> CUBE `Function=0`
** `BSTART.TMATMUL.ACC` -> CUBE `Function=2`
** `BSTART.ACCCVT` -> CUBE `Function=8`
...
v0.57.1 TEPL mode/function allocation policy:
...
* The catalog assigns exactly 98 unique selectors across Modes 0 through 3;
  the remaining 30 selector positions are reserved and illegal.
...
Unassigned TEPL mode/function pairs are reserved/illegal in canonical v0.57.1.
...
* Generic mixed TMA textual forms remain illegal in canonical v0.57 sources. The legal public spelling is the assigned
  `BSTART.<alias>` form.
* The PTO map is frozen in `isa/v0.57/state/pto_encoding_map.json`: exactly 120 operations have one TMA,
...
* `BSTART.TLOAD` / `BSTART.TSTORE` (named TMA Functions 0/1):
...
** are header-driven block commands (no SIMT body; do not use `B.TEXT`).
* `BSTART.ACCCVT` (named CUBE-family Function 8):
** conversion controls are carried by the operation-applicable `B.DATR` fields `Sat`, `Canonicalize`, `DataType`, `RMode`, and `Layout`.
