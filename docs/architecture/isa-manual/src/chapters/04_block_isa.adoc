...
* TMA, CUBE, and TEPL name binary encoding families. In v0.56, the TMA family has no generic legal assembly form: only the exact `BSTART.TLOAD`, `BSTART.TSTORE`, and `BSTART.TMOV` forms are legal.
...
* TMA `Function=3..31` encodings are reserved and MUST raise an illegal-instruction exception. They are not instruction forms and MUST NOT contribute to legal-form or executable-coverage denominators.
