...
** `4..6`: reserved
** `7`: illegal
  ACC is singleton implicit state selected by CUBE function identity; it has no `B.IOT` code or depth/index.
...
* `TMATMUL` / `TMATMUL.ACC` use `SrcTile0/SrcTile1` as the two tile inputs; the output is implicit ACC state.
...
** write the singleton implicit accumulator; B.IOT carries no ACC source or destination code.
...
** `TMATMUL` starts ACC state and `TMATMUL.ACC` may repeat; `ACCCVT` is not a v0.58 direct CUBE operation.
** are header-driven block commands (no SIMT body; do not use `B.TEXT`).
* `BSTART.TEPL`:
