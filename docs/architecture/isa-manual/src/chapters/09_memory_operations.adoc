...
[[memory-model]]
=== Memory model and ordering (v0.57 current)
...
[[memory-channels]]
=== BCC and MTC channels (v0.57 current)

v0.57 explicitly models two architectural memory issue channels inside one TSO ordering domain:
...
Legacy raw `TPREFETCH` text is archived only and is not a live encoded operation.
...
`BSTART.TPREFETCH` is active in the v0.57 TMA selector space as `Function=3`. Its memory
...
v0.57 adds the 32-bit `CASB/CASH/CASW/CASD` encodings. They atomically read `[SrcL]`, compare against `SrcR`,
...
Layout and dimension contract (v0.57 current profile for TMA):
...
* `B.DATR.Layout` encodes the layout selector for the current TMA operation.
...
* Raw material that contradicts group-overlap semantics from archived sources is not part of canonical v0.57 behavior; selector/group
  space not defined by the catalog is reserved/illegal.

Canonical v0.57 profile constraints:

* Applicable `B.DATR` fields plus `B.IOR` are mandatory in descriptor-rich `TLOAD/TSTORE` headers.
* TMA execution consumes the typed block start and applicable `B.CATR/B.DATR/B.DIM/B.IOR/B.IOT` state.
...
* Raw `group=2` material and broad fixed-pipe/TCVT metadata flows are reserved/illegal in canonical v0.57 until a future
  catalog revision assigns explicit semantics.

Preferred v0.57 textual form for tile memory blocks is:
...
For completeness: v0.57 also permits non-memory typed TMA headers `BSTART.TMOV <DataType>`
...
`->t<1KB>`). ACC is implicit and is never encoded as a `B.IOT` identity.
...
* Archived VREG-only raw local-register fragments are not part of canonical v0.57 behavior.

Bridged address-formation contract (canonical v0.57):
...
Linx platforms classify memory addresses into types. This v0.57 manual uses two
high-level types:
...
The v0.57 Linx toolchain and emulator model are **little-endian**: the least-significant byte of a multi-byte value is
...
The catalog includes `*.PCR` forms that compute an address relative to the current PC/TPC (as defined by the v0.57 catalog).
These forms are used by toolchains for efficient access to nearby constants and for position-independent code patterns.

[[memory-atomics]]
=== Atomics (overview)

The v0.57 catalog includes load-reserved/store-conditional (`LR.*`/`SC.*`), compare-and-swap (`CAS*`), and read-modify-write style atomic operations
with qualifier suffixes (for example `.aq`, `.rl`). The precise memory ordering semantics are defined by
`isa/v0.57/state/memory_model.json` and the architectural model implemented by the toolchain/QEMU/RTL.
...
The v0.57 catalog defines two fence instructions:
...
Bring-up definition (v0.57 current profile):
...
Semantics (v0.57 current profile):
