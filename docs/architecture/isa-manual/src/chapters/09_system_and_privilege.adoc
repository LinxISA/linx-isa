...
+|===
+
+NOTE: v0.3 text previously used `E_INST(EC_ILLEGAL)` and `E_INST(EC_PARAM)` informally without allocating numeric encodings. v0.4-draft allocates concrete `EC_*` values above.
+
