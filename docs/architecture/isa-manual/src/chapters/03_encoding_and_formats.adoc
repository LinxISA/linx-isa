...
The v0.57 encoding model defines:
...
Within canonical `MPAR`/`MSEQ` shader-kernel bodies, v0.57 uses the unified `lx64` execution rule:
...
The authoritative encoding details live in `isa/v0.57/linxisa-v0.57.json`:
...
[NOTE]
====
A complete set of encoding diagrams for all 747 instruction forms is available in the
