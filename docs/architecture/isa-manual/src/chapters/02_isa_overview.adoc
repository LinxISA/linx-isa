...
This chapter provides a narrative overview of the major instruction families in the v0.57 catalog. The complete,
...
The canonical shader-kernel body space in v0.57 is a unified 64-bit `lx64` encoding space used by `MPAR` and `MSEQ`
kernel bodies:
...
The instruction reference chapter enumerates the encodings present in the v0.57 catalog.
...
* CUBE-family typed forms such as `BSTART.TMATMUL`, `BSTART.TMATMUL.ACC`, and `BSTART.ACCCVT`,
* `BSTART.TEPL`, whose raw header carries `Mode[1:0]` and `Function[4:0]`; the logical selector is
  `(Mode << 5) | Function`.

Selector values that are not explicitly assigned by the v0.57 catalog are reserved and must not acquire ad hoc semantics.
