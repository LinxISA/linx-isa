[[system]]
== System and privileged architecture (v0.57)

This chapter defines the v0.57 privileged architecture: Access Control Rings (ACR), privileged system registers
...
Bring-up conventions (v0.57 current profile):
...
Informative software role mapping (v0.57 current profile):
...
|===
+
NOTE: earlier draft text previously used `E_INST(EC_ILLEGAL)` and `E_INST(EC_PARAM)` informally without allocating numeric encodings. Canonical `v0.57` inherits the concrete `EC_*` values above.
+
...
`ASSERT_FAIL (52)` has two distinct producers in v0.57:
...
[NOTE]
====
These items remain platform-specific policy inputs on top of the canonical v0.57 privileged architecture.
