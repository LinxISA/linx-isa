...
* For TEPL templates (`TemplateKind.class=1`), `TPL_ID_PRESENT` MUST be `1` and `TPL_ID` MUST carry `(Mode << 5) | Function`.
...
* TEPL-carried templates (`TemplateKind.class=1` / `BSTART.TEPL`) are accelerator-driven (not uop-expanded). TEPL is the Mode/Function encoding carrier; the selected operation executes on VEC or SFU.
...
*** `class=1`: TEPL templates with `id = (Mode << 5) | Function` (0..127).
