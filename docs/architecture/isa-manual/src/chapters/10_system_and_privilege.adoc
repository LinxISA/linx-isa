[[system]]
== System and privileged architecture (v0.58)

This chapter defines the v0.58 privileged architecture: Access Control Rings (ACR), privileged system registers
...
Bring-up conventions (v0.58 current profile):
...
Informative software role mapping (v0.58 current profile):
...
. Populate trap reporting registers:
  * For a SYNC_SERVICE_REQUEST: set `TRAPNO_ACRm.E = 1`, then write `TRAPNO_ACRm` / `TRAPARG0_ACRm`.
...
Template replay flags (bring-up):

* Implementations SHOULD expose a single EBARG register `EBARG_TPLFLAGS_ACRm` with template trap/replay flags. The bit layout is defined in <<ssr-ebarg>> (see `EBARG_TPLFLAGS_ACRn`).
...
* For VEC/SFU operations on the TEPL carrier (`TemplateKind.class=1`), `TPL_ID_PRESENT` MUST be `1` and `TPL_ID` MUST carry `(Mode << 5) | Function`.
* `TPL_REDO_OK=1`: handler may restart the template from the beginning only when no externally visible partial side effects exist.
* `TPL_RESUME_OK=1`: template supports and permits resume via an `EBSTATE` snapshot.
* `TPL_PHASE` standard meanings:
  - `0`: decode/validate
  - `1`: executing
  - `2`: finishing/commit
  - `3`: reserved
* For `FENTRY`/`FEXIT`/`FRET.*` templates, phase zero is validation, phase one is the ordered event stream defined in <<templates-frame-contract>>, and phase two is the final boundary transfer/retirement row.
* For recoverable `FENTRY`/`FEXIT`/`FRET.*` phase-one traps, `TPL_REDO_OK` MUST be `0` and `TPL_RESUME_OK` MUST be `1`.
* `TPL_RESUME_OK=0` is the required terminal exception for the post-seal template-integrity fail-stop in <<system-template-integrity-fail>>; it does not permit rollback of committed template effects.
* VEC/SFU operations (`TemplateKind.class=1`) are accelerator-driven (not uop-expanded). TEPL is only their Mode/Function encoding carrier.
  - `TPL_PHASE` is defined by the selected Mode/Function operation but MUST respect the top-level meanings; `TPL_PHASE=0` is decode/validate only and MUST NOT be used once execution begins.
  - `TPL_REDO_OK` MUST be `0` once execution begins.
  - `TPL_RESUME_OK` is defined by the selected Mode/Function operation.
  - `TPL_DIRTY` is set if the selected VEC or SFU engine state actually progresses and requires restore.
  - `ECSTATE.BI` for VEC/SFU carrier traps: `BI=0` when `TPL_PHASE=0`, and `BI=1` when `TPL_PHASE=1/2`.
...
|===
+
NOTE: earlier draft text previously used `E_INST(EC_ILLEGAL)` and `E_INST(EC_PARAM)` informally without allocating numeric encodings. Canonical `v0.58` inherits the concrete `EC_*` values above.
+
...
[[system-block-exception-retention]]
===== Recoverable exceptions during block execution

If a recoverable exception occurs after a block enters execution, hardware MUST freeze the faulting block and retain all
architectural state required to continue it.  At minimum this includes block-register contents, execution and sequence
progress, predicate/control state, outstanding-operation state, and any restart cursor defined by the block family.

Software MAY export retained state with `ESAVE`.  An implementation MUST provide an independent save path or reserve
enough registers, queue entries, and issue capacity for `ESAVE` to make progress when the faulting block occupies all
ordinary resources.

* VECTOR blocks MAY raise architecturally defined recoverable internal exceptions.
* CUBE blocks do not raise CUBE-owned recoverable exceptions after execution starts.
* TLSU blocks do not raise TLSU-owned recoverable exceptions.  Translation, permission, and bus errors produced by the
  memory system remain `E_DATA` exceptions.
* VECTOR/CUBE first-use exceptions occur before execution and are not block-internal exceptions.

`TMA` is a historical name for TLSU and is not used by the active profile.

A TLSU global-memory address supplied through `B.IOR` is a data virtual address in the initiating ACR.  While `IOTCR`
leaves I/O translation disabled, it uses the current task's ordinary CPU translation and permission context.  It is
not implicitly an I/O virtual address; IOMMU translation applies only when `IOTCR` explicitly enables the I/O address
space.  A failed TLSU
memory beat reports the original virtual address in `TRAPARG0` and follows the precise `E_DATA` path.

[[system-extension-first-use]]
===== VECTOR/CUBE first-use exception

The current unreleased v0.58 main contract defines a precise pre-execution exception so an ACR1 kernel can allocate or restore extension
context only when an ACR2 task first needs it.

The exact trap envelope is:
...
This producer uses raw `CAUSE=4`; it does not change the profile-specific cause formats or trap numbers used by other
producers.  Archived misspellings of `EC_PERM` are not active aliases.

VECTOR first-use applies to `BSTART.MPAR`, `BSTART.MSEQ`, `BSTART.VPAR`, `BSTART.VSEQ`, and their compressed forms.
It does not apply to the TEPL assembly aliases `BSTART.VEC` or `BSTART.SFU`.  CUBE membership is derived from canonical
PTO operations whose `family` and `engine` are both `CUBE`.

The processor MUST complete legal decode and ACR permission checks before this check.  It MUST take first use before
BARG/BSTATE mutation, context allocation, queue admission, memory issue, or any other architectural effect.  An invalid
encoding or invalid block target therefore retains its normal exception priority.  The saved PC identifies the original
block header, and retry after software clears/restores the corresponding state MUST execute that header exactly once.

`ECONFIG_ACR1.V` controls VECTOR and `ECONFIG_ACR1.C` controls CUBE for ACR2.  Handling one extension MUST NOT clear the
other enable bit.  Software MUST distinguish `NEVER_USED`, `SAVED_NOT_RESTORED`, and `LIVE`; one used/not-used flag is
insufficient for lazy restore.

[[system-template-integrity-fail]]
===== Template-integrity `ASSERT_FAIL`

`ASSERT_FAIL (52)` has two distinct producers in v0.58:

[%header,cols="2,1,1,1,3"]
|===
|Producer |ECONFIG[3] maskable |Local fixup |`BI` |Continuation

|Executed `ASSERT` instruction |yes |existing instruction behavior |0 |existing precise ASSERT behavior
|Frame-template integrity fail-stop |no |forbidden |1 |forbidden; source-context reuse is reset-only
|===

The frame-template integrity producer is hardware-generated. It is not execution
of the `ASSERT` instruction, is not masked by `ECONFIG[3]`, and does not use
instruction-local fixup-label routing. It follows the default `E_ASSERT`
managing-ring route and writes:

* `TRAPNUM = ASSERT_FAIL (52)`
* `TRAPNO.CAUSE = 0`
* `TRAPNO.E = 0`
* `TRAPNO.ARGV = 1`
* `TRAPARG0 = source template PC/TPC`
* `ECSTATE.BI = 1`
* `EBARG_TPC = source template PC/TPC` as diagnostic state only
* `TPL_PHASE = current phase (1 or 2)`
* `StepIndex = next event/final boundary not committed`
* `TPL_DIRTY = 1`
* `TPL_REDO_OK = 0`
* `TPL_RESUME_OK = 0`

`EBSTATE.Template.FatalReason` is:
...
The fatal snapshot is the architectural state after the last committed
frame-template event and before the failing event or `FINAL`. No committed
effect or trace is removed; the failing and younger operations have no effect.

Fatal handling MUST stop source-context issue, commit, wakeup, redirect, `FINAL`
transfer, and new lease acquisition; snapshot the exact state, owner, token, and
pending invalidation transactions; advance the relevant template/row/load
generations; cancel every uncommitted row and request; invalidate VLOAD data,
VTGT proof, token, and queued transfer; obtain quiescence from ROB, LSU,
target-validation, block-control, recovery, trace, redirect/transfer, and the
lease directory so no stale action can commit, wake, transfer, or recreate a
lease; and only then release the lease and create one `RELEASED_AFTER_ABORT`
acknowledgement for every pending matching invalidation. No terminal
acknowledgement is observable before quiescence.

The managing ring may inspect and log the frozen state, then reset the LXCPU.
It cannot return to the source context: `ACRE RRAT_RESTORE` and
`ACRE RRAT_DEFAULT` both raise `E_INST(EC_PARAM)` in the manager and leave the
source fail-stopped. Platform reset is the only reuse boundary and creates a new
context generation. Reset during fatal teardown must first provide global
quiescence. No pre-reset acknowledgement may appear after the new context
generation, and reset never restores pre-template state.
...
For retained frame-template execution, a complete dirty `EBSTATE` snapshot
includes the exact owner identity, phase and `StepIndex` cursors, sealed VLOAD
state when present, validation token, retained lease, and complete pending
invalidation transaction/status set. `ACRE RRAT_RESTORE` must resume with those
identities unchanged. The implementation MUST NOT renumber, merge, drop, or
reacquire pending invalidation entries, and the lease directory must remain
discoverable while the source context is suspended.
...
*** `class=1`: VEC/SFU operations on the TEPL carrier with `id = (Mode << 5) | Function` (0..127).
...
DMA-style engines (including `TLSU` engines) MUST perform address translation using the IOMMU when enabled.
...
[NOTE]
====
These items remain platform-specific policy inputs on top of the canonical v0.58 privileged architecture.
