...
`BSTART.TMATMUL`, `BSTART.TMATMUL.ACC`, and `BSTART.ACCCVT` are named
CUBE-family forms. Their assigned function values are part of the frozen
0.57.1 selector map; no generic executable `BSTART.CUBE` form exists.
...
* Applicable `B.DATR` fields plus `B.IOR` are **mandatory** for quantization controls and scalar bindings.
