...
Address translation (v0.58 current profile, preserving the v0.2 MMU baseline):
...
* Legacy trap-save singleton naming from pre-v0.2 drafts is not part of canonical v0.58 artifacts.
...
NOTE: The v0.58 `reg5` table also includes special spellings used by the assembler/disassembler for queue operations
...
They are optionally written by `B.DIM` / `C.B.DIM*` and are intended to carry block-local loop bounds and accelerator metadata. At each block start, an omitted LB has effective value one; an explicit write, including zero, replaces that default.

Rules (bring-up convention):

* LB registers are initialized at block start and used within the current block.
* Presence is used only for duplicate-write rejection and trap recovery; operation legality consumes the effective values.
...
=== Shader-kernel lane policy (v0.58 current)

For canonical shader-kernel execution (`BSTART.MPAR` / `BSTART.MSEQ`), v0.58 locks an explicit inactive-lane policy:
...
[[progmodel-vpred]]
=== Block-control predicate and kernel EXEC mask (v0.58)

v0.58 defines two predicate domains and keeps them distinct:
...
* **Kernel EXEC mask** (`p`): a 64-bit architectural source/destination used inside `MPAR`/`MSEQ` kernel bodies.
  Vector instructions are implicitly predicated by `p`. PTO ISA 0.58.3 removes
  the standalone `B.Z` / `B.NZ` forms and reserves their former encodings.
...
* Canonical shader-kernel special operands are:
  ...
  - `p` and `zero`.
* Archived raw operand names `TE..TH` and `TO1/TO2/TO3` are not assigned meaning in canonical v0.58 and remain
  reserved/unassigned.
...
* Direct `v.*` writes into scalar/group-domain destinations remain illegal unless the instruction is an explicitly
  ...
  Example:
** `v.add vt#1, ri0, ->t` is illegal in canonical v0.58.
** `v.rdadd vt#1, ->t` is legal because `V.RD*` is an explicit reduction family.

SIMT group model (v0.58 current profile):
...
* Canonical compilers must preserve the meanings `LB0 = lanes per group` and `LB1 = group count`; `LB0` is not reinterpreted
  as total logical threads in canonical `v0.58`.
...
Linx defines a tile register file used by accelerator-oriented *tile blocks* (for example `BSTART.TLOAD`,
`BSTART.TMATMUL`, `BSTART.VEC`, `BSTART.SFU`, `BSTART.MPAR`, `BSTART.MSEQ`, `BSTART.VPAR`, and `BSTART.VSEQ`).
...
Bring-up conventions (v0.58 current profile, with the v0.2 trap ABI baseline):
...
named **ACR0..ACR15**, which are organized as a tree rooted at **ACR0** (see <<system-acr-tree>>). In the v0.58 current
profile:
...
NOTE: In the v0.58 current profile (same as the v0.2 baseline), floating-point exceptions do **not** trap; they only update sticky flags in `FCSR`
