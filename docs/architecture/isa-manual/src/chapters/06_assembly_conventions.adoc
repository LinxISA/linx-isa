...
Many arithmetic and load instructions have destinations described as `->{t, u, Rd}` in the v0.57 catalog, indicating:
...
The v0.57 ISA tables use several common conventions:
...
* In canonical v0.57 shader kernels, bridged global-memory forms spell the access domain explicitly: `l.*.brg` or `v.*.brg`.

Refer to `instructions[].asm` in `isa/v0.57/linxisa-v0.57.json` for the full syntax used by each instruction form.
