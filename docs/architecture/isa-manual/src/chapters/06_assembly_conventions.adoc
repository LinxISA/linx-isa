...
* In canonical v0.4 shader kernels, bridged global-memory forms spell the access domain explicitly: `l.*.brg` or `v.*.brg`.

Refer to `instructions[].asm` in `isa/v0.4/linxisa-v0.4.json` for the full syntax used by each instruction form.
