...
Many arithmetic and load instructions have destinations described as `->{t, u, Rd}` in the v0.58 catalog, indicating:
...
The v0.58 ISA tables use several common conventions:
...
* In canonical v0.58 shader kernels, bridged global-memory forms spell the access domain explicitly: `l.*.brg` or `v.*.brg`.

Refer to `instructions[].asm` in `isa/v0.58/linxisa-v0.58.json` for the full syntax used by each instruction form.
