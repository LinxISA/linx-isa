---
// Auto-generated instruction documentation
// Do not edit by hand

[[inst_hl_sbi_pr]]
==== HL.SBI.PR

*Syntax:* hl.sbi.pr SrcD, [SrcR, simm], ->{t, u, Rd}

*Description:* HL.SBI.PR snapshots its scalar sources, forms its encoded address, and stores one aligned little-endian 1-byte value.

*Encoding:*

image::encodings/enc_hl_sbi_pr.svg[HL.SBI.PR encoding,width=800]

*Uop-Class:* STA/PRE_INDEX

*Compression:* HL48 (len=48)

*Uop-Class payload:* `{"addr_mode": "PRE_INDEX", "agu_kind": "STA", "note": "User confirmed all stores are AGU/STA + STD", "source": "group_rule", "split": ["AGU", "STD"], "uop_kind": "AGU"}`
