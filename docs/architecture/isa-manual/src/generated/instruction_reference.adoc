...
|===
|Mnemonic |Len |Decode |Assembly |Notes
|`B.CATR` |32 |- |`B.CATR {trap, atomic, <aq, rl, aqrl>, far, dr}` |PTO ISA 0.57.1 control attributes. Only DR[26], trap[19], far[18], atom[17], aq[16], and rl[15] are variable.
|===
...
|===
|Mnemonic |Len |Decode |Assembly |Notes
|`B.DATR` |32 |- |`B.DATR {layout, datatype, padvalue_or_byteid, cmode, rmode, sat, canonicalize}` |PTO ISA 0.57.1 data attributes. PadValueOrByteId is an opcode-defined two-bit union; bits 19:18 are reserved zero.
|===
...
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, <last>, ->DstTile<Size>` |PTO ISA 0.57.1 one-source descriptor. Inactive source and reuse fields encode zero.
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, SrcTile1<.reuse>, <last>, ->DstTile<Size>` |PTO ISA 0.57.1 two-source descriptor. imm4 3..9 selects 128B..8KiB; ACC is never encoded as DstTile.
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, <last>` |PTO ISA 0.57.1 one-source destination-free descriptor. All inapplicable fields encode zero.
|`B.IOT` |32 |- |`B.IOT <last>, ->DstTile<Size>` |PTO ISA 0.57.1 zero-source destination descriptor. Inactive source and reuse fields encode zero.
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, SrcTile1<.reuse>, <last>` |PTO ISA 0.57.1 two-source destination-free descriptor. DstTile and imm4 encode zero; zero does not name an output.
|===
...
|`BSTART.ACCCVT` |32 |- |`BSTART.ACCCVT DataType` |Named CUBE-family Function=8. ACCCVT reads implicit ACC and uses B.DATR conversion controls before publishing an ordinary Tile.
...
|`BSTART.MGATHER` |32 |- |`BSTART.MGATHER DataType` |TMA Function=4. Memory gather uses B.DATR/B.IOR/B.IOT descriptor state.
...
|`BSTART.MSCATTER` |32 |- |`BSTART.MSCATTER DataType` |TMA Function=5. Memory scatter uses B.DATR/B.IOR/B.IOT descriptor state.
...
|`BSTART.TEPL` |32 |- |`BSTART.TEPL Mode, Function, DataType` |PTO ISA 0.57.1 TEPL selector: logical_selector=(Mode<<5)\|Function. There is no legacy 10-bit TileOpcode decode.
...
|`BSTART.TLOAD` |32 |- |`BSTART.TLOAD DataType` |TMA Function=0. Uses B.DATR layout/pad and B.IOR stride/base plus B.IOT tile binding.
|`BSTART.TMATMUL` |32 |- |`BSTART.TMATMUL DataType` |Named CUBE-family Function=0. Requires B.DIM (LB0/LB1/LB2 as m,n,k) and tile bindings via B.IOT.
|`BSTART.TMATMUL.ACC` |32 |- |`BSTART.TMATMUL.ACC DataType` |Named CUBE-family Function=2. Requires B.DIM (LB0/LB1/LB2 as m,n,k) and tile bindings via B.IOT.
...
|`BSTART.TSTORE` |32 |- |`BSTART.TSTORE DataType` |TMA Function=1. Uses B.DATR layout/pad and B.IOR stride/base plus B.IOT tile binding.
