...
|===
|Mnemonic |Len |Decode |Assembly |Notes
|`BSTART CALL` |32 |- |`BSTART.CALL <br_label>` |Bare BSTART.CALL preserves ra. A returning call must be preceded by SETRET or C.SETRET with an explicit return label.
|`HL.BSTART CALL` |48 |- |`HL.BSTART.CALL <br_label>` |Bare HL.BSTART.CALL preserves ra. A returning call must be preceded by SETRET or C.SETRET with an explicit return label.
