...
|===
|Mnemonic |Len |Decode |Assembly |Notes
|`B.CATR` |32 |- |`B.CATR {trap, atomic, <aq, rl, aqrl>, far, dr}` |canonical split of legacy B.CATR/B.DATR control fields. Bits 25:15 are reserved zero.
|===
...
|===
|Mnemonic |Len |Decode |Assembly |Notes
|`B.DATR` |32 |- |`B.DATR {layout.{canon, normal}, datatype, padvalue, cmode, rmode, sat}` |canonical split of legacy B.CATR/B.DATR data fields. CMode is 3 bits; RMode is 3 bits; Sat is bit 31.
|===
...
|`B.HINT` |32 |- |`B.HINT TRACE.{begin, end}` |Bits 31:16 are reserved zero.
|===
...
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, <last>, ->DstTile<Size>` |canonical one-input immediate-size tile descriptor; function=101 means only SrcTile0 is valid. v0.57 excludes DstTile=111 from this destination-producing form.
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, SrcTile1<.reuse>, <last>` |v0.57 two-input source-only tile descriptor required by multi-descriptor PTO expansions. DstTile=111 denotes no output and the unused size field is canonically zero.
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, SrcTile1<.reuse>, <last>, ->DstTile<Size>` |canonical two-input immediate-size tile descriptor. SrcTile0/1 are independent 6-bit fields; imm4 encodes 0B..512KB. v0.57 excludes DstTile=111 from this destination-producing form.
|`B.IOT` |32 |- |`B.IOT SrcTile0<.reuse>, <last>` |v0.57 one-input source-only tile descriptor required by multi-descriptor PTO expansions. DstTile=111 denotes no output and the unused size field is canonically zero.
|`B.IOT` |32 |- |`B.IOT <last>, ->DstTile<Size>` |canonical no-input immediate-size tile descriptor; function=110 means only the destination allocation is valid.
|===
...
|`BSTART.CUBE` |32 |- |`BSTART.CUBE Function, DataType` |Generic CUBE selector form for unnamed functions. Functions 0/1/2/4/5/6/8/16/17/18/20/21/22 decode through their canonical named forms.
|`BSTART.FIXP` |32 |- |`BSTART.FIXP TileOp, DataType` |-
|`BSTART.FP` |32 |- |`BSTART.FP RET` |Bits 31:15 are reserved zero.
|`BSTART.FP` |32 |- |`BSTART.FP ICALL` |Bits 31:15 are reserved zero.
|`BSTART.FP` |32 |- |`BSTART.FP COND, <label>` |-
|`BSTART.FP` |32 |- |`BSTART.FP IND` |Bits 31:15 are reserved zero.
|`BSTART.FP` |32 |- |`BSTART.FP DIRECT, <label>` |-
|`BSTART.FP` |32 |- |`BSTART.FP CALL, <label>` |-
|`BSTART.FP` |32 |- |`BSTART.FP FALL<, fixup_label>` |-
|`BSTART.MGATHER` |32 |- |`BSTART.MGATHER DataType` |Named TMA-family Function=4. Memory gather uses B.ARG/B.IOR/B.IOT descriptor state; detailed PTO selector manifest is defined out-of-band for v0.57.
|`BSTART.MGATHER.CAS` |32 |- |`BSTART.MGATHER.CAS DataType` |Named TMA-family Function=8. Per-element atomic compare-and-swap gather; detailed PTO selector manifest is defined out-of-band for v0.57.
|`BSTART.MGATHER.MASK` |32 |- |`BSTART.MGATHER.MASK DataType` |Named TMA-family Function=6. Masked memory gather; detailed PTO selector manifest is defined out-of-band for v0.57.
|`BSTART.MPAR` |32 |- |`BSTART.MPAR <VS8, VS16>` |-
|`BSTART.MSCATTER` |32 |- |`BSTART.MSCATTER DataType` |Named TMA-family Function=5. Memory scatter uses B.ARG/B.IOR/B.IOT descriptor state; detailed PTO selector manifest is defined out-of-band for v0.57.
|`BSTART.MSCATTER.MASK` |32 |- |`BSTART.MSCATTER.MASK DataType` |Named TMA-family Function=7. Masked memory scatter; detailed PTO selector manifest is defined out-of-band for v0.57.
|`BSTART.MSEQ` |32 |- |`BSTART.MSEQ <VS8, VS16>` |-
|`BSTART.STD` |32 |- |`BSTART.STD COND, <label>` |-
|`BSTART.STD` |32 |- |`BSTART.STD ICALL` |Bits 31:15 are reserved zero.
|`BSTART.STD` |32 |- |`BSTART.STD FALL<, fixup_label>` |-
|`BSTART.STD` |32 |- |`BSTART.STD RET` |Bits 31:15 are reserved zero.
|`BSTART.STD` |32 |- |`BSTART.STD IND` |Bits 31:15 are reserved zero.
|`BSTART.STD` |32 |- |`BSTART.STD CALL, <label>` |-
|`BSTART.STD` |32 |- |`BSTART.STD DIRECT, <label>` |-
|`BSTART.SYS` |32 |- |`BSTART.SYS FALL<, fixup_label>` |-
|`BSTART.TEPL` |32 |- |`BSTART.TEPL TileOpcode, DataType` |Tile opcode selects PTO template ops. Canonical established assignments require TileOpcode[9:6]=0 and pack TileOpcode[5]=Mode with TileOpcode[4:0]=Function; unassigned values remain reserved for future template extension.
|`BSTART.TGEMV` |32 |- |`BSTART.TGEMV DataType` |Named CUBE-family Function=16. Matrix-vector multiply; requires B.DIM and CUBE tile bindings.
|`BSTART.TGEMV.ACC` |32 |- |`BSTART.TGEMV.ACC DataType` |Named CUBE-family Function=18. Matrix-vector multiply-accumulate; requires B.DIM and CUBE tile bindings.
|`BSTART.TGEMV.BIAS` |32 |- |`BSTART.TGEMV.BIAS DataType` |Named CUBE-family Function=17. Matrix-vector multiply with bias; requires B.DIM and CUBE tile bindings.
|`BSTART.TGEMVMX` |32 |- |`BSTART.TGEMVMX DataType` |Named CUBE-family Function=20. Micro-scaled matrix-vector multiply; requires B.DIM and CUBE tile bindings.
|`BSTART.TGEMVMX.ACC` |32 |- |`BSTART.TGEMVMX.ACC DataType` |Named CUBE-family Function=22. Micro-scaled matrix-vector multiply-accumulate; requires B.DIM and CUBE tile bindings.
|`BSTART.TGEMVMX.BIAS` |32 |- |`BSTART.TGEMVMX.BIAS DataType` |Named CUBE-family Function=21. Micro-scaled matrix-vector multiply with bias; requires B.DIM and CUBE tile bindings.
...
|`BSTART.TMATMUL.BIAS` |32 |- |`BSTART.TMATMUL.BIAS DataType` |Named CUBE-family Function=1. Matrix multiply with bias; requires B.DIM and CUBE tile bindings.
|`BSTART.TMATMULMX` |32 |- |`BSTART.TMATMULMX DataType` |Named CUBE-family Function=4. Micro-scaled matrix multiply; requires B.DIM and CUBE tile bindings.
|`BSTART.TMATMULMX.ACC` |32 |- |`BSTART.TMATMULMX.ACC DataType` |Named CUBE-family Function=6. Micro-scaled matrix multiply-accumulate; requires B.DIM and CUBE tile bindings.
|`BSTART.TMATMULMX.BIAS` |32 |- |`BSTART.TMATMULMX.BIAS DataType` |Named CUBE-family Function=5. Micro-scaled matrix multiply with bias; requires B.DIM and CUBE tile bindings.
...
|`BSTART.TPREFETCH` |32 |- |`BSTART.TPREFETCH DataType` |Named TMA-family Function=3. TPREFETCH has the same address, layout, memory, fault, ordering, and restart contract as TLOAD, with destination binding/allocation/writeback effects removed.
...
|`L.BSTART.FP` |64 |- |`L.BSTART.FP COND, <label>` |-
|`L.BSTART.FP` |64 |- |`L.BSTART.FP DIRECT, <label>` |-
...
|`L.BSTART.FP` |64 |- |`L.BSTART.FP FALL<, fixup_label>` |-
|`L.BSTART.STD` |64 |- |`L.BSTART.STD DIRECT, <label>` |-
...
|`L.BSTART.STD` |64 |- |`L.BSTART.STD COND, <label>` |-
|`L.BSTART.STD` |64 |- |`L.BSTART.STD FALL<, fixup_label>` |-
|`L.BSTART.SYS` |64 |- |`L.BSTART.SYS FALL<, fixup_label>` |-
|===
...
|===
|Mnemonic |Len |Decode |Assembly |Notes
|`CASB` |32 |- |`casb<.{aq, rl, aqrl}> [SrcL], SrcR, SrcD, ->{t, u, Rd}` |v0.57 32-bit compare-and-swap byte: old=[SrcL]; if old==SrcR then [SrcL]=SrcD; RegDst=old. Supports aq/rl ordering and has no far flag.
|`CASD` |32 |- |`casd<.{aq, rl, aqrl}> [SrcL], SrcR, SrcD, ->{t, u, Rd}` |v0.57 32-bit compare-and-swap doubleword: old=[SrcL]; if old==SrcR then [SrcL]=SrcD; RegDst=old. Supports aq/rl ordering and has no far flag.
|`CASH` |32 |- |`cash<.{aq, rl, aqrl}> [SrcL], SrcR, SrcD, ->{t, u, Rd}` |v0.57 32-bit compare-and-swap halfword: old=[SrcL]; if old==SrcR then [SrcL]=SrcD; RegDst=old. Supports aq/rl ordering and has no far flag.
|`CASW` |32 |- |`casw<.{aq, rl, aqrl}> [SrcL], SrcR, SrcD, ->{t, u, Rd}` |v0.57 32-bit compare-and-swap word: old=[SrcL]; if old==SrcR then [SrcL]=SrcD; RegDst=old. Supports aq/rl ordering and has no far flag.
...
|===
|Mnemonic |Len |Decode |Assembly |Notes
|`DMA` |32 |- |`dma [SrcL], SrcR` |v0.57 encoding for the inherited DMA operation: copy 64 bytes from the source address in SrcL to the destination address in SrcR and complete before submission.
|===
