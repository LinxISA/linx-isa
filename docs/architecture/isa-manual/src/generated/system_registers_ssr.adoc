...
// Source: linxisa-v0.58.json (state.system_registers)
[[ssr-table]]
...
|`0x0010` |`TIME` |RO |Monotonic time counter in nanoseconds. Reads return elapsed monotonic time modulo 2^64 and are not affected by wall-clock adjustments.
...
|`0xnf08` |`IPENDING_ACRn` |RO |Interrupt pending bitmap.
|`0xnf09` |`TOPEI_ACRn` |RO |Top pending interrupt ID.
...
|`0xnf30` |`XBINFO_ACRn` |RW |Cross-block info register. Provides the CAC_TABLE base address for the selected ACR in the v0.58 profile.
...
[[ssr-econfig-contract]]
==== ECONFIG field contract (v0.58)
...
|`V` |`32` |VECTOR first-use exception enable.
|`C` |`33` |CUBE first-use exception enable.
|===

Reserved ranges: `[31:4]`, `[63:34]`.

Reserved writes are `must-zero`; reserved reads return `zero`.
