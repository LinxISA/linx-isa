...
10. Kernel global memory access uses bridged forms only: `l.*.brg` and `v.*.brg`.
11. Tiles remain general-purpose intermediate-state storage; the first rendering profile uses 4KB `1024x1` row-major SoA fragment/state carriers.
12. TAU execution is tile-to-tile only; small state uses descriptors and hot/large state uses StateTiles.
13. Contradictory legacy raw fragments are archived history only and are reserved or illegal in canonical v0.4.
14. Privileged state, trap envelopes, template replay flags, and body-fetch rules follow the v0.4 manual and state catalog.
