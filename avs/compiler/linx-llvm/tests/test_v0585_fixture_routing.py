...
class V0585FixtureRoutingTest(unittest.TestCase):
    def test_current_lane_routes_only_v0585_fixture(self) -> None:
        self.assertTrue((CURRENT_ASM / "41_v0585_isa_forms.s").is_file())
        self.assertFalse((CURRENT_ASM / "41_v0583_isa_forms.s").exists())
        self.assertFalse((CURRENT_ASM / "41_v057_isa_forms.s").exists())
        self.assertTrue((ARCHIVED_ASM / "41_v057_isa_forms.s").is_file())

        runner = RUNNER.read_text()
        self.assertIn("41_v0585_isa_forms)", runner)
        self.assertNotIn("41_v0583_isa_forms)", runner)
