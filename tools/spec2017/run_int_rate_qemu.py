...
def _apply_qemu_debug_env(
    ...
    *,
    ...
    qemu_frame_page_fast: bool = False,
    qemu_frame_single_restore_host_load: bool = False,
    ...
    if qemu_frame_page_fast:
        qemu_env["LINX_QEMU_FRAME_PAGE_FAST"] = "1"
    if qemu_frame_single_restore_host_load:
        qemu_env["LINX_QEMU_FRAME_SINGLE_RESTORE_HOST_LOAD"] = "1"
...
QEMU_ACRE_TRACE_ARGS = {
    "qemu_acre_trace_pc": "LINX_QEMU_ACRE_TRACE_PC",
    "qemu_acre_trace_pc_lo": "LINX_QEMU_ACRE_TRACE_PC_LO",
    "qemu_acre_trace_pc_hi": "LINX_QEMU_ACRE_TRACE_PC_HI",
    "qemu_acre_trace_bpc": "LINX_QEMU_ACRE_TRACE_BPC",
    "qemu_acre_trace_bpc_lo": "LINX_QEMU_ACRE_TRACE_BPC_LO",
    "qemu_acre_trace_bpc_hi": "LINX_QEMU_ACRE_TRACE_BPC_HI",
    "qemu_acre_trace_count_lo": "LINX_QEMU_ACRE_TRACE_COUNT_LO",
    "qemu_acre_trace_count_hi": "LINX_QEMU_ACRE_TRACE_COUNT_HI",
    "qemu_acre_trace_target": "LINX_QEMU_ACRE_TRACE_TARGET",
    "qemu_acre_trace_rra": "LINX_QEMU_ACRE_TRACE_RRA",
    "qemu_acre_trace_trapnum": "LINX_QEMU_ACRE_TRACE_TRAPNUM",
    "qemu_acre_trace_limit": "LINX_QEMU_ACRE_TRACE_LIMIT",
    "qemu_acre_trace_code_bytes": "LINX_QEMU_ACRE_TRACE_CODE_BYTES",
}
...
def _qemu_acre_trace_from_args(args: argparse.Namespace) -> dict[str, str]:
    trace: dict[str, str] = {}
    for attr, env_name in QEMU_ACRE_TRACE_ARGS.items():
        value = str(getattr(args, attr, "") or "").strip()
        if value:
            trace[env_name] = value
    for attr, env_name in QEMU_ACRE_TRACE_BOOL_ARGS.items():
...
def _run_qemu(
    ...
    _apply_qemu_debug_env(
        ...
        qemu_frame_page_fast=qemu_frame_page_fast,
        qemu_frame_single_restore_host_load=qemu_frame_single_restore_host_load,
    ...
    qemu_info = {
        ...
        "qemu_frame_page_fast": bool(qemu_frame_page_fast),
        "qemu_frame_single_restore_host_load": bool(qemu_frame_single_restore_host_load),
        ...
        "pc_watch": pc_watch,
        "pc_watch_seen": pc_watch["seen"],
        "pc_watch_line_count": pc_watch["line_count"],
        "pc_watch_last": pc_watch["last"],
        "pc_watch_samples": pc_watch["samples"],
        "pc_watch_ring_seen": pc_watch["ring_seen"],
        "pc_watch_ring_count": pc_watch["ring_count"],
        "pc_watch_ring_entry_count": pc_watch["ring_entry_count"],
        "pc_watch_last_ring": pc_watch["last_ring"],
        "pc_watch_last_ring_entry": pc_watch["last_ring_entry"],
        "pc_watch_last_ring_fields": pc_watch["last_ring_fields"],
        "pc_watch_last_ring_entry_fields": pc_watch["last_ring_entry_fields"],
        "pc_watch_ring_entry_samples": pc_watch["ring_entry_samples"],
        "pc_watch_call_trace_ring_seen": pc_watch["call_trace_ring_seen"],
        "pc_watch_call_trace_ring_count": pc_watch["call_trace_ring_count"],
        "pc_watch_call_trace_ring_entry_count": pc_watch["call_trace_ring_entry_count"],
        "pc_watch_last_call_trace_ring": pc_watch["last_call_trace_ring"],
        "pc_watch_last_call_trace_ring_entry": pc_watch["last_call_trace_ring_entry"],
        "pc_watch_last_call_trace_ring_fields": pc_watch["last_call_trace_ring_fields"],
        "pc_watch_last_call_trace_ring_entry_fields": pc_watch[
            "last_call_trace_ring_entry_fields"
        ],
        "pc_watch_call_trace_ring_entry_samples": pc_watch[
        ...
        "acre_trace_seen": acre_trace["seen"],
...
def _heartbeat_frame_stats_summary(line: str) -> dict[str, Any]:
    ...
    return {
        ...
        "page_fast_fentry": _decimal_or_none(fields.get("fr_page_fast_fentry")),
        "page_fast_restore": _decimal_or_none(fields.get("fr_page_fast_restore")),
    }
...
def _tlb_fill_hot_summary(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "last_hits": _decimal_or_none(fields.get("last_hits")),
        "slot_hits": _decimal_or_none(fields.get("slot_hits")),
        "slots": _decimal_or_none(fields.get("slots")),
        "top0_count": _decimal_or_none(fields.get("top0_count")),
...
def _acre_trace_summary(text: str) -> dict[str, Any]:
    lines = re.findall(r"^LINX_ACRE_(?:TRACE|REGS|CODE) .*$", text, flags=re.MULTILINE)
    samples: list[dict[str, Any]] = []
    int_keys = {
        "count",
        "mgr",
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--qemu-frame-page-fast",
        action="store_true",
        default=_env_bool("LINX_SPEC_QEMU_FRAME_PAGE_FAST", False),
        help="Set LINX_QEMU_FRAME_PAGE_FAST=1 to use the opt-in same-page multi-register frame fast path.",
    )
    parser.add_argument(
        "--qemu-frame-single-restore-host-load",
        action="store_true",
        default=_env_bool("LINX_SPEC_QEMU_FRAME_SINGLE_RESTORE_HOST_LOAD", False),
        help=(
            "Set LINX_QEMU_FRAME_SINGLE_RESTORE_HOST_LOAD=1 to use cached "
            "host loads only for the one-register FRET.STK fast path."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--qemu-acre-trace",
        action="store_true",
        default=_env_bool("LINX_SPEC_QEMU_ACRE_TRACE", False),
        help="Set LINX_QEMU_ACRE_TRACE=1 for ACRE trap-return queue-state diagnostics.",
    )
    parser.add_argument("--qemu-acre-trace-pc", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_PC", ""))
    parser.add_argument("--qemu-acre-trace-pc-lo", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_PC_LO", ""))
    parser.add_argument("--qemu-acre-trace-pc-hi", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_PC_HI", ""))
    parser.add_argument("--qemu-acre-trace-bpc", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_BPC", ""))
    parser.add_argument("--qemu-acre-trace-bpc-lo", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_BPC_LO", ""))
    parser.add_argument("--qemu-acre-trace-bpc-hi", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_BPC_HI", ""))
    parser.add_argument("--qemu-acre-trace-count-lo", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_COUNT_LO", ""))
    parser.add_argument("--qemu-acre-trace-count-hi", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_COUNT_HI", ""))
    parser.add_argument("--qemu-acre-trace-target", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_TARGET", ""))
    parser.add_argument("--qemu-acre-trace-rra", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_RRA", ""))
    parser.add_argument("--qemu-acre-trace-trapnum", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_TRAPNUM", ""))
    parser.add_argument("--qemu-acre-trace-limit", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_LIMIT", ""))
    parser.add_argument("--qemu-acre-trace-code-bytes", default=os.environ.get("LINX_SPEC_QEMU_ACRE_TRACE_CODE_BYTES", ""))
    parser.add_argument(
        "--qemu-acre-trace-regs",
        action="store_true",
        default=_env_bool("LINX_SPEC_QEMU_ACRE_TRACE_REGS", False),
