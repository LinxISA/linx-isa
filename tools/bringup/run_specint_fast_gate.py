...
def _suite_command(
    *,
    ...
    if qemu_mmu_cache_victim and forward_qemu_mmu_cache_victim:
    ...
    if qemu_tb_hot and forward_qemu_tb_hot:
...
def _write_md(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        f"- qemu_mmu_cache_victim: `{str(bool(summary.get('qemu_mmu_cache_victim', False))).lower()}`",
        ...
        f"- qemu_tb_hot: `{str(bool(summary.get('qemu_tb_hot', False))).lower()}`",
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=_env_bool(
            "SPEC_QEMU_MMU_CACHE_VICTIM",
            _env_bool("LINX_SPEC_QEMU_MMU_CACHE_VICTIM", False),
        ),
        help="Forward QEMU's opt-in direct-map victim page-walk cache entry.",
    )
    parser.add_argument(
        "--template-chain",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=_env_bool(
            "SPEC_QEMU_TB_HOT",
            _env_bool("LINX_SPEC_QEMU_TB_HOT", False),
        ),
        help="Forward QEMU's opt-in hot TB lookup PC heartbeat sketch.",
    )
    ...
    runner_has_qemu_mmu_cache_victim = _runner_supports_option(
        runner, "--qemu-mmu-cache-victim"
    )
    ...
    runner_has_qemu_tb_hot = _runner_supports_option(runner, "--qemu-tb-hot")
    ...
    if args.qemu_frame_page_fast and not runner_has_qemu_frame_page_fast:
        raise SystemExit(
            "error: local SPEC matrix runner does not support "
    ...
    if args.qemu_mmu_cache_victim and not runner_has_qemu_mmu_cache_victim:
        raise SystemExit(
            "error: local SPEC matrix runner does not support "
            "--qemu-mmu-cache-victim; update tools/spec2017/run_stage_qemu_matrix.py "
            "or rerun without the MMU cache victim switch"
        )
    ...
    if args.qemu_tb_hot and not runner_has_qemu_tb_hot:
        raise SystemExit(
            "error: local SPEC matrix runner does not support "
            "--qemu-tb-hot; update tools/spec2017/run_stage_qemu_matrix.py "
            "or rerun without the TB hot switch"
        )
    ...
    for suite in suites:
        ...
        for unit in _suite_execution_units(suite, args.transports):
            ...
            cmd = _suite_command(
                ...
                qemu_mmu_cache_victim=args.qemu_mmu_cache_victim,
                ...
                forward_qemu_mmu_cache_victim=runner_has_qemu_mmu_cache_victim,
                ...
                forward_qemu_tb_hot=runner_has_qemu_tb_hot,
