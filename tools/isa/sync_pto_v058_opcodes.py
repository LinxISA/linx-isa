...
def old_owned_lines() -> dict[Path, set[int]]:
    spec = load(ROOT / "isa/v0.57/linxisa-v0.57.json")
    result: dict[Path, set[int]] = {}
    for instruction in spec["instructions"]:
        if not instruction.get("pto_source_form_id"):
            continue
...
def opcode_line(form: dict[str, Any], encoding: list[dict[str, Any]], variant: str | None) -> str:
    projected = dict(form)
...
def main() -> int:
    ...
    if command_catalog.get("form_count") != 96:
        raise ValueError("PTO 0.58 must publish exactly 96 command forms")

    removals = old_owned_lines()
    ...
    for form in command_catalog["forms"]:
        ...
        generated[target].append(opcode_line(form, form["encoding"], None))
        for variant in form.get("encoding_variants", []):
            generated[target].append(
                opcode_line(form, variant["encoding"], str(variant["name"]))
            )

    for target, new_lines in generated.items():
        relative_v057 = Path("opcodes") / target.name
        removed = removals.get(relative_v057, set())
        original = target.read_text(encoding="utf-8").splitlines()
        retained = [line for number, line in enumerate(original, 1) if number not in removed]
        retained.extend(["", "# PTO ISA 0.58.0 canonical command forms", *new_lines])
        target.write_text("\n".join(retained) + "\n", encoding="utf-8")
    return 0
