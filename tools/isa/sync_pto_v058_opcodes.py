...
GENERATED_MARKER = "# PTO ISA 0.58.5 canonical scalar and command forms"
...
def opcode_line(
    ...
    meta: dict[str, Any] = {
        ...
        "note": form.get("semantic_summary", "PTO ISA 0.58.5 canonical form."),
...
def main() -> int:
    ...
    if command_catalog.get("form_count") != 76:
        raise ValueError("PTO 0.58.5 must publish exactly 76 command forms")
    if scalar_catalog.get("form_count") != 466:
        raise ValueError("PTO 0.58.5 must publish exactly 466 scalar forms")
    if reservations.get("reservation_count") != 46:
        raise ValueError("PTO 0.58.5 must publish exactly 46 extension reservations")
